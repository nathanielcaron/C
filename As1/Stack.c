 ...
 *
 *
 *	Created by Jean-Philippe Legault and Nathaniel Caron
 ******************************************************/
 ...
 *		pointer to the stack
 *		pointer to the size
 *		pointer to location to store the peeked value
 *
 * returns:
	...
	while(!stop_execution) {
		...
		if( false == is_whitespace(input_instruction) ) {
			...
			if(input_instruction == 'u') {
				int input_value = 0;
				scanf("%d", &input_value);
				bool result = push(&stack[0], &stack_current_size, stack_max_size, input_value);
				if(!result) {
					printf("failed push\n");
				}
 				else {
				...
				int to_return = 0;
				bool result = pop(&stack[0], &stack_current_size, &to_return);
				...
				int to_return = 0;
				bool result = peek(&stack[0], &stack_current_size, &to_return);
				if(!result) {
					printf("failed peek\n");
				}
 				else {
